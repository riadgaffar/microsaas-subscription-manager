...
public class UserClientTests {
  ...
  void shouldReturn403ForInvalidUserAuthorities() {
    ResponseEntity<String> responseEntity = restTemplate.getForEntity(createURLWithPort("/api/authorities?email=user@user.com"), String.class);
  ...
  void shouldReturnAuthoritiesRolesForAdminAndSuperAdminValidUser() {
    ...
    String[] authorities = restTemplate.getForObject(createURLWithPort("/api/authorities?email=user1@example.com"), String[].class);
  ...
  void shouldReturnAUserDetailsWhenDataIsSaved() {
    ResponseEntity<String> response = restTemplate.getForEntity(createURLWithPort("/api/users/0"), String.class);
  ...
  void shouldNotReturnAUserWithAnUnknownId() {
    ResponseEntity<String> response = restTemplate.getForEntity(createURLWithPort("/api/users/1000"), String.class);
  ...
  void shouldReturnASubscriptionForAUser() {
    String url = createURLWithPort("/api/users/{userId}/subscriptions/{subscriptionId}");
  ...
  void shouldNotReturnASubscriptionForAUserWithAnUnknownSubscriptionId() {
    ResponseEntity<String> response = restTemplate.getForEntity(createURLWithPort("/api/users/0/subscriptions/1000"), String.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
    assertThat(response.getBody()).isBlank();
  }
  ...
  void shouldReturnAllSubscriptionForAUser() {
    String url = createURLWithPort("/api/users/{userId}/subscriptions");
  ...
  void shouldRegisterANewUserAndAddANewSubscription() {
    ...
    URI newUserLocation = restTemplate
        .postForLocation(createURLWithPort("/api/users"), newUser);
    ...
    String addSubscriptionUrl = String.format(createURLWithPort("/api/users/%d/subscription"), retrievedUser.getId());
  ...
  void shouldNotRegisterANewUserWithDuplicateEmail() {
    ...
    ResponseEntity<Void> response = restTemplate
        .postForEntity(createURLWithPort("/api/users"), newUser, Void.class);
  ...
  void shouldAddAndDeleteASubscriptionForAnExistingUser() {
    ...
    URI userSubscriptionLocation = restTemplate.postForLocation(createURLWithPort(addSubscriptionUrl), newSubscription, 3);
